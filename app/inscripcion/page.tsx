...
import { courseMeta } from "../data/program";
...
export default function RegistrationPage() {
  // "$2,500 USD" → símbolo / cifra / divisa, para conservar la jerarquía tipográfica.
  const [, currencySymbol, amount, currencyCode] = courseMeta.investment.match(/^(\D*)([\d.,]+)\s*(\w*)$/) ?? [];

  return (
    <main>
      <section className="page-hero registration-hero"><div><p className="eyebrow"><span /> COHORTE 2026</p><h1>Ocho lugares.<br /><em>Una señal clara.</em></h1><p>Cuéntanos brevemente sobre tu práctica. El equipo académico te contactará con requisitos, disponibilidad y próximos pasos.</p></div><aside className="investment-card"><p>Inversión por participante</p><strong><span>{currencySymbol}</span>{amount}<small> {currencyCode}</small></strong><ul><li>40 h de precurso virtual</li><li>40 h de mentoría presencial</li><li>12 meses de seguimiento</li><li>Acceso a materiales digitales</li></ul></aside></section>
      <section className="section form-section"><div><p className="section-index">SOLICITUD DE INFORMACIÓN</p><h2>Tu práctica,<br />tu punto de partida.</h2><p>Envía tus datos y el equipo académico te contactará. No se realizará ningún cobro desde este sitio.</p><div className="contact-note"><span>Contacto directo</span><a href={`mailto:${courseMeta.contactEmail}`}>{courseMeta.contactEmail}</a></div></div><RegistrationForm /></section>
